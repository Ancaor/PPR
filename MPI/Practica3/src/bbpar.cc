...
main (int argc, char **argv) {
  ...
  MPI_Comm_split(MPI_COMM_WORLD, color_carga, _rank, &comunicadorCarga);
  MPI_Comm_split(MPI_COMM_WORLD, color_carga, _rank, &comunicadorCota);

  siguiente = (_rank+1)%_size;
  anterior = (_rank-1+_size)%_size;
 ...
 // cout << "proceso : " << _rank << "ha obtenido carga" << endl;
  ...
  fin = Inconsistente(tsp0);
    ...
    EquilibrarCarga(pila, fin, solucion);
    //cout << "proceso : " << _rank << "ha obtenido carga en el ciclo" << endl;
		if (!fin) pila.pop(nodo);
